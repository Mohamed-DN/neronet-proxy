...
router.post('/', async (req, res) => {
  try {
    ...
    await logAuditEvent({
      ...
      message: `Created pre-auth key ${key.key_prefix} (reusable: ${key.is_reusable})`,
      ipAddress: req.ip
    });
...
router.get('/', async (req, res) => {
  try {
    ...
    const keys = await listPreAuthKeys(req.user.id, isSuperAdmin);
...
router.delete('/:id', async (req, res) => {
  try {
    const isSuperAdmin = req.user.role === 'super-admin';
    const success = await revokePreAuthKey(req.params.id, req.user.id, isSuperAdmin);
