...
const testDbPath = path.resolve(__dirname, '../../data/test_route_sync.db');
...
describe('Route delivery', () => {
  ...
  beforeEach(() => {
    getDatabase().prepare('DELETE FROM network_routes').run();
  });
  ...
  it('withholds a route whose gateways are all unhealthy', async () => {
    ...
    getDatabase().prepare('UPDATE nodes SET is_healthy = 0 WHERE id = ?').run(gateway.assigned_node_id);
    ...
    getDatabase().prepare('UPDATE nodes SET is_healthy = 1 WHERE id = ?').run(gateway.assigned_node_id);
  });
  ...
  it('withholds a route whose gateway is quarantined', async () => {
    ...
    getDatabase().prepare('UPDATE nodes SET is_quarantined = 1 WHERE id = ?').run(gateway.assigned_node_id);
    assert.strictEqual((await sync(client.assigned_node_id)).body.routes.length, 0);
    getDatabase().prepare('UPDATE nodes SET is_quarantined = 0 WHERE id = ?').run(gateway.assigned_node_id);
  });
  ...
  it('resolves peer health at delivery time, not at creation time', async () => {
    ...
    getDatabase().prepare('UPDATE nodes SET is_healthy = 0 WHERE id = ?').run(second.assigned_node_id);
    ...
    getDatabase().prepare('UPDATE nodes SET is_healthy = 1 WHERE id = ?').run(second.assigned_node_id);
  });
